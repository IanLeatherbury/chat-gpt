...
// Function to send user messages to the API and return the response
...
function Toast({ show, message }: { show: boolean; message: string }) {
    return (
        <div className={`${show ? 'flex' : 'hidden'} fixed bottom-0 left-0 right-0 items-center justify-center mb-4 z-50`}>
...
export default function ChatPrompt({ onMessageSubmit }: ChatPromptProps) {
    ...
    // Function to display toast with a message for 3 seconds
    ...
    const handleSubmit = async (e: React.SyntheticEvent) => {
        ...
        if (textareaRef.current) {
            // Get message text and clear textarea
            const messageText = textareaRef.current.value;
            textareaRef.current.value = '';

            try {
                // Send user message and update state
                const userMessage = { role: 'user', content: messageText };
                const updatedMessages = [...messages, userMessage];
                onMessageSubmit(userMessage, 0);

                // Get assistant response and update state
    ...
    // Render the ChatPrompt component
    return (
                        ...
                        <div className="absolute inset-x-0 bottom-0 flex justify-between py-2 pl-3 pr-2 rounded-lg dark:bg-gray-800">
                            ...
                            <div className="flex-shrink-0">
                                ...
                                >
                                    {isProcessing ? (
                                        <>
                                            <svg className="w-5 h-5 mr-2 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none"
                                                viewBox="0 0 24 24">
